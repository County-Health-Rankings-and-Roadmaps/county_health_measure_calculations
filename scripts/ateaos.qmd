...
```{r load data }
library(tidyverse)
tmeasuredata = haven::read_sas("P:/CH-Ranking/Data/Cumulative Analytic Datasets/2024 Data/t_measure_data.sas7bdat")

# grab the health factors only (ie where category / factor_parent == 2) 
tfactor = haven::read_sas("P:/CH-Ranking/Data/Cumulative Analytic Datasets/t_factor.sas7bdat") %>% filter(year == 2024 & factor_parent == 2) 


# grab the focus areas that are health factors 
tfocus = haven::read_sas("P:/CH-Ranking/Data/Cumulative Analytic Datasets/t_focus_area.sas7bdat") %>% filter(year == 2024 & focus_area_parent %in% tfactor$factor_id) %>% select(focus_area_id, focus_area_numerator, focus_area_denominator)

# grab the ranked health factors only 
tmeasure = haven::read_sas("P:/CH-Ranking/Data/Cumulative Analytic Datasets/t_measure.sas7bdat") %>% filter(year == 2024 & measure_parent %in% tfocus$focus_area_id & measure_type == 0)
...
county_summary = countymeasures %>% 
  ...
  mutate(nmed = ifelse(measure_id ==9, 0.073, #for 24 only, remove after confirm match 
                       ifelse(measure_id == 11, 0.245, #for 24 only; remove after confirm match 
                              median(raw_value, na.rm = TRUE))), 
...
tstate = haven::read_sas("P:/CH-Ranking/Data/Cumulative Analytic Datasets/2024 Data/t_state_data.sas7bdat") %>% 
...
```{r some code to compare w sas }
score %>% filter(measure_id == 4 & state_fips == "01" & county_fips == "001") %>% select(ntlz, sz, atoez, aofsz, atoecomp, aofscomp, atescore, aofscore)
```
...
# compare to sas bin 
binsas = haven::read_sas('P:/CH-Ranking/Data/2024/9 Results/Analysis/ateaos/bin_temp.sas7bdat')



bincombo = merge(binsas, bin, by = c("measure_id", "state_fips","county_fips"))

bincombo = bincombo %>% mutate(
  diffaos = ifelse(is.na(binaos.x) & !is.na(binaos.y) | 
                  !is.na(binaos.x) & is.na(binaos.y) | 
                  binaos.x - binaos.y != 0, 1, NA),
  diffate = ifelse(is.na(binate.x) & !is.na(binate.y) | 
                  !is.na(binate.x) & is.na(binate.y) | 
                  binate.x - binate.y != 0, 1, NA)) %>% 
  select(binaos.x, binaos.y, binate.x, binate.y, diffaos, diffate, atescore.x, atescore.y, aosscore, aofscore, atoecomp.x, atoecomp.y, aofscomp.x, aofscomp.y, ci_low.x, ci_high.x)

  
  
# these are the spots to check!!!! 

uhoh = bincombo %>% filter(diffate == 1 | diffaos ==1)
...
#quick check 
# prat %>% filter(state_fips == "09" & measure_id == 4)
```
...
# compare to sas F 
fsas = haven::read_sas('P:/CH-Ranking/Data/2024/9 Results/Analysis/ateaos/f_temp.sas7bdat')

fcombo = merge(f, fsas, by = c("measure_id", "state_fips","county_fips"))

fcombo = fcombo %>% mutate(
  diffaos = ifelse(is.na(binaos.x) & !is.na(binaos.y) | 
                  !is.na(binaos.x) & is.na(binaos.y) | 
                  binaos.x - binaos.y != 0, 1, NA),
  diffate = ifelse(is.na(binate.x) & !is.na(binate.y) | 
                  !is.na(binate.x) & is.na(binate.y) | 
                  binate.x - binate.y != 0, 1, NA)) %>% 
  select(binaos.x, binaos.y, binate.x, binate.y, diffaos, diffate, 
         atoez.x, atoez.y, aofsz.x, aofsz.y, aosprat.x, aosprat.y, 
         ateprat.x, ateprat.y,
         measure_id, state_fips, county_fips) #, binaos1, binate1)

  
  
# these are the spots to check!!!! 

uhoh = fcombo %>% filter(diffate == 1 | diffaos ==1) %>% 
  filter(measure_id %in% uhohcodes$measure_id & 
           state_fips %in% uhohcodes$state_fips & 
           county_fips %in% uhohcodes$county_fips)

uhohcodes = uhoh %>% select(measure_id, state_fips, county_fips)
...
f1 <- f %>%
  mutate(
    ...
    binaos = case_when(
      ...
      binboth == 1 & measure_id %in% c(9, 11) ~ NA,
      TRUE ~ binaos
    ),
    binate = case_when(
      binboth == 1 & measure_id %in% c(9, 11) ~ 1,
      binboth == 1 & sz <= ntlz ~ NA,
    ...
    # Remove binaos for measure_id 9 and 11
    binaos = if_else(measure_id %in% c(9, 11), NA, binaos)
  )


# compare to sas F 
fsas = haven::read_sas('P:/CH-Ranking/Data/2024/9 Results/Analysis/ateaos/f1_temp.sas7bdat')

fcombo = merge(f1, fsas, by = c("measure_id", "state_fips","county_fips"))
...
# consider moving f1 into SAS and then using SAS Code from there 
...
newcount <- f1 %>%
  ...
  filter(!(measure_id %in% c(9,11,124)))
...
#tot <- ranked1 %>% 
 # select(state_fips, county_fips, measure_id, binaosnew, sumaos) %>% 
 # right_join(f1, by = c("state_fips", "county_fips", "measure_id"))
...
# compare to sas F 
totsas = haven::read_sas('P:/CH-Ranking/Data/2024/9 Results/Analysis/ateaos/tot_temp.sas7bdat')

totcombo = merge(tot, totsas, by = c("measure_id", "state_fips","county_fips"))

totcombo = totcombo %>% mutate(
  diffaos = ifelse(is.na(binaos.x) & !is.na(binaos.y) | 
                  !is.na(binaos.x) & is.na(binaos.y) | 
                  binaos.x - binaos.y != 0, 1, NA),
  diffate = ifelse(is.na(binate.x) & !is.na(binate.y) | 
  ...
  select(binaos.x, binaos.y, binate.x, binate.y, diffaos, diffate, 
         binaos3.x, binaos3.y,
         binboth.x, binaosnew.x, binaosnew.y, sumaos.x, sumaos.y, 
         measure_id, state_fips, county_fips) #, binaos1, binate1)
...
uhoh = totcombo %>% filter(diffate == 1 | diffaos ==1) 

uhoh = totcombo %>% filter(binaosnew.x != binaosnew.y)

uhoh = totcombo %>% filter(sumaos.x != sumaos.y)


# Step 2: Final cleanup and renaming columns
totally <- tot %>%
...
kate = haven::read_sas("P:/CH-Ranking/Data/2024/9 Results/Analysis/ateaos/ateaos_final_kb.sas7bdat")
