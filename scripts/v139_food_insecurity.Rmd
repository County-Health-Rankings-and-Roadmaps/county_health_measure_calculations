...
# Prepare fa_cnt
fa_cnt_prepped <- fa_cnt %>%
  mutate(
    statecode = str_sub(fips, 1, 2),       # first 2 chars of fips
    countycode = str_sub(fips, -3, -1)     # last 3 chars of fips
  )

# Prepare fa_st
fa_st_prepped <- fa_st %>%
  mutate(
    statecode = fips,       # already 2-char state code
    countycode = "000"      # all zeroes for state-level
  )

# Combine
fa <- bind_rows(fa_st_prepped, fa_cnt_prepped)
...
- numerator = 47,389,000
- rawvalue = 0.143

```{r}
fa_3 <- fa_2 %>% 
  add_row(statecode = "00", countycode = "000", rawvalue = 0.143,
          numerator = 47389000, .before = 1)
...
# quick check
```{r}
mb = haven::read_sas("P:/CH-Ranking/Data/2026/4 To be verified by/v139.sas7bdat")

summary(mb$v139_rawvalue)
summary(mb$v139_numerator)

summary(v139_1$v139_rawvalue)
summary(v139_1$v139_numerator)
