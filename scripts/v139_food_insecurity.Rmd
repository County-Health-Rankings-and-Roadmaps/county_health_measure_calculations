---
title: "v139 Food Insecurity - R2025"
author: "GL"
...
raw_dir <- "C:/01_CHRR/data/r2025/raw/mmg/MMG_Data_v3"

# county data
fa_cnt <- openxlsx::read.xlsx(file.path(raw_dir, "MMG2024_2019-2022_Data_ToShare_v3.xlsx"),
                        sheet = "County") %>% 
  janitor::clean_names() %>% 
  filter(year == 2022) %>% 
...
# state data
fa_st <- openxlsx::read.xlsx(file.path(raw_dir, "MMG2024_2019-2022_Data_ToShare_v3.xlsx"),
                        sheet = "State") %>% 
  janitor::clean_names() %>% 
  filter(year == 2022) %>% 
  select(fips, rawvalue = overall_food_insecurity_rate,
...
```{r}
write_csv(v139_1, 
          paste0("../measure_datasets/", "v139", "_r2025",
                 ".csv"),
          na = "")
```
