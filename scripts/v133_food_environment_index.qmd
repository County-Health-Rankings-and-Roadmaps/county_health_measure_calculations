---
title: "v133 Food environment Index"
author: how
format: html
editor: visual
---
...
Map the Meal Gap: https://map.feedingamerica.org/ This data has to be requested via this request form: https://feedingamerica.az1.qualtrics.com/jfe/form/SV_5tJt5m9K62hRC6N
...
Limited Access to Healthy Food: The % with limited access to healthy foods is the "numerator." Use variables LALOWI1_10/Pop2010 to calculate Limited Access to Healthy Foods (This is the same as measure v083 - limited access to healthy foods. So instead of downloading data from the source, it is possible to use data from folder three for v083 if it has already been verified.)

Food Insecurity: The % considered food insecure is the "denominator." Use column titled "Overall Food Insecurity Rate (1 year)" and column titled "Year" (for 2022) (This is the same as measure v139 - food insecurity. So instead of downloading data from the source, it is possible to use data from folder three for v139 if it has already been verified.)
...
```         
 a) The z score for the numerator: (numeratorvalue – averagenumvalue)/sd(numeratorvalue)

 b) The z score for the denominator: (denomvalue – averagedenomvalue)/sd(denomvalue) 
...
#food insecurity 
#make sure the statecode and countycode cols are loaded correctly
fi = read.csv(here("measure_datasets/v139_r2026.csv")) %>% 
  mutate(
    statecode = str_pad(statecode, width = 2, side = "left", pad = "0"),
    countycode = str_pad(countycode, width = 3, side = "left", pad = "0")
  )

#limited access to healthy food 
# this measure has not been updated since rankings 2022 
la = read.csv(here("measure_datasets/v083.csv")) %>% 
  mutate(
    statecode = str_pad(statecode, width = 2, side = "left", pad = "0"),
    countycode = str_pad(countycode, width = 3, side = "left", pad = "0")
  )
...
# add in the standardized fipscodes 
v133 = sc %>% 
  select(v133_rawvalue, statecode, countycode, v133_numerator, v133_denominator) %>%
   mutate(
    statecode = str_pad(statecode, width = 2, side = "left", pad = "0"),
    countycode = str_pad(countycode, width = 3, side = "left", pad = "0")
  )
...
```{r}
# Merge with fips dataset, keeping all rows in fips
v133_merged <- fips %>%
  left_join(v133, by = c("statecode", "countycode"))
...
```{r}
mb = haven::read_sas("P:/CH-Ranking/Data/2026/3 Data Calculated needs checking/v133.sas7bdat")
summary(mb$v133_rawvalue)
summary(v133_merged$v133_rawvalue)

summary(mb$v133_numerator)
summary(v133_merged$v133_numerator)


comparison <- mb %>%
  full_join(v133_merged, by = c("statecode", "countycode"), suffix = c("_mb", "_v133")) %>% 
  mutate(diffrv = abs(v133_rawvalue_mb - v133_rawvalue_v133),
diffnum = abs(v133_numerator_mb - v133_numerator_v133),
  diffdenom = abs(v133_denominator_mb - v133_denominator_v133))


temp = comparison %>% filter(diffdenom>0.001)
```

# save the measure_dataset

```{r}
write.csv(v133_merged, file = here("measure_datasets/v133_R2026.csv"), row.names = FALSE)
