---
title: "v138 Drug overdose deaths (subgroups) - Ranking 2025"
author: "GL"
...
```{r}
pop_all_2 <- 
  update_pop_CT_new_sub61(pop_all, num_yrs = 3, state = FALSE) %>% # set state=FALSE if we want to keep CT state pop same as Census pop estimates, TRUE if we want fabricated CT state pop
  glimpse()
```
...
```{r}
CT_mort_new <- 
  get_CT_from_CT_mort_sub61(CT_mort_2016to2022, year_lst = 2020:2022, icd_code_lst=icd_code_lst)
```
