---
title: "NCHS v147 Life Expectancy (LE) (subgroups) - LE R2025"
...
```{r}
process_pop_v147_other <- function(year, by_race_TF = TRUE){
...
## function: process mortality data for v147 subgroups

```{r}
process_mort_v147_other <- function(year, by_race_TF = TRUE){
...
## Census pop data by race and age cat 1-18 for 2020-2022

```{r}
pop_agecat_1_18 <- bind_rows(
  process_pop_v147_other(2020), 
  process_pop_v147_other(2021),
  process_pop_v147_other(2022) %>% 
    fix_CT_pop(process_pop_v147_other(2021))
  ) %>% 
...
```{r}
mort_all <- bind_rows(
  process_mort_v147_other(2020),
  process_mort_v147_other(2021),
  process_mort_v147_other(2022)
  ) %>% 
...
v147_other_2 <- v147_other %>% 
  add_flag_CT(vnum = "v147_race", old="A", new = "U")
