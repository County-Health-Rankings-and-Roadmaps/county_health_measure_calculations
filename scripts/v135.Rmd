---
title: "v135 Injury deaths - Ranking 2025"
author: "GL"
...
```{r}
pop_all_2 <- 
  update_pop_CT_new(pop_all, num_yrs = 5, state = FALSE) %>% # set state=FALSE if we want to keep CT state pop same as Census pop estimates, TRUE if we want fabricated CT state pop
  glimpse()
```
...
```{r}
CT_mort_new <- 
  get_CT_from_CT_mort(CT_mort_2016to2022, year_lst = 2018:2022, icd_code_lst=icd_code_lst)
```
