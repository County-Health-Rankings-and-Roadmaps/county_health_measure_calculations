---
title: "v021 High School Graduation - R2025"
author: "HOW"
format: html
editor: visual
---
...
<https://eddataexpress.stage.ed.gov/download>

Steps to download "Adjusted Cohort Graduation Rate" data at LEA Level:

1.  go to https://eddataexpress.stage.ed.gov/download
...
3.  Select:
    ...
    Level: Local Education Agency

    School Year: 2021-2022

    Subgroups: All Students -\> All Students in LEA

    Other Filters: File Specification -\> 150 ACGR (Graduation Rate), 151 ACGR Cohorts

    Data Group: 695 ACGR-4year, 696 ACGR cohorts-4 year

4.  Click on the "DOWNLOAD DATA" button to download CSV and data note

Steps to download "Adjusted Cohort Graduation Rate" data at School Level for Hawaii:
...
3.  Select:
    ...
    Subgroups: All Students -\> All Students in School
...
The information to assign districts to the county-level was downloaded from https://nces.ed.gov/ccd/elsi/tableGenerator.aspx.

Under Table Row select "District", and for Years select "2021-22".

Then for Table Columns under Information/Basic Information select the following:

"Agency Name",

"County Number \[District\] (2021-22)",

"Agency ID - NCES Assigned \[District\] (Latest available year)"

After importing the data, subset such that CATEGORY = ALL.
...
Use the mid-point of each reported graduation rate interval, and estimate the numerator. If a graduation rate is given as "PS", then that observation is not included in either county-level or state-level calculations.

If the most specific district graduation rate in a given county is a range of greater than 5 percentage points then the county's graduation rate is not reported. If a county has a district graduation rate that is as specific or more than 5 percentage points, then all districts with reported graduation rates, no matter the level of specificity, are included in the calculation of that county's graduation rate.

All data with reported graduation rates, regardless of its level of specificity, are included in the calculation of state-level data. (As noted above, observations with graduation rate = 'PS' are not included in calculations.)

School-level data are used for Hawaii because Hawaii only has 1 school district and county-rates cannot be calculated with their district-level data.

For the 2025 release, we utilized data from the NCES webpage (<https://nces.ed.gov/programs/coe/indicator/coi/high-school-graduation-rates)> to obtain the U.S. average adjusted cohort graduation rate (ACGR) of 87% for public high school students during the 2021-2022 school year.

# Notes: 

ME, NM, and OK do not have data present this year.
...
if leanm = 'State Charter Schools- Mountain Education Center School' then delete; \[or use **leaid 000001300214**\]

The 2021-2022 Public School Universe was used to assign Hawaii schools to counties (based on school NCES codes) from [https://nces.ed.gov/ccd/elsi/tableGenerator.aspx ](https://nces.ed.gov/ccd/elsi/tableGenerator.aspx)
...
```{r}
ed = readr::read_csv(here("raw_data/EDFacts/edfactsdownload.csv"))
hi = readr::read_csv(here("raw_data/EDFacts/edfactsdownload_hi.csv"))
names = readr::read_csv(here("raw_data/EDFacts/leacrosswalk.csv"), skip = 6)
schoolnames = readr::read_csv(here("raw_data/EDFacts/schoolcrosswalk.csv"), skip = 6)

```

# merge names with LEA level data

```{r}
edn = merge(ed, names, by.x ="NCES LEA ID" , by.y = "Agency ID - NCES Assigned [District] Latest available year", all.x = TRUE)

#check to see which ones did not merge correctly 
...
# add in the cleaned fipscodes (loaded at top of document)
...
write.csv(cnsf, file = here("measure_datasets/v021_R2025.csv"))
```
